document.addEventListener('DOMContentLoaded', () => {
    ...
    // Font size interactions
    let currentFontSize = 100; // Track percentage
    ...
    fontIncrease.addEventListener('click', () => {
        currentFontSize = Math.min(200, currentFontSize + 10); // Don't go above 200%
        document.body.style.fontSize = `${currentFontSize}%`;
    });

    // Configure marked wrapper for highlight.js syntax highlighting
    marked.setOptions({
    ...
    // Get the lab URL from the querystring
    const urlParams = new URLSearchParams(window.location.search);
    let labUrl = urlParams.get('lab');

    // If no URL provided, show a helpful message
    if (!labUrl) {
        contentDiv.innerHTML = '<div class="error">No lab URL specified.<br><br>Please provide a "?lab=" query parameter with the raw GitHub URL.<br>Example: <code>?lab=https://raw.githubusercontent.com/...</code></div>';
        return;
    }

    // Automatically convert standard GitHub URLs to Raw URLs
    if (labUrl.startsWith('https://github.com/')) {
        // Change the domain
        labUrl = labUrl.replace('https://github.com/', 'https://raw.githubusercontent.com/');
        // Remove /blob/ or /tree/
        labUrl = labUrl.replace('/blob/', '/').replace('/tree/', '/');
        
        // If the URL points to a folder instead of a specific markdown file, assume README.md
        if (!labUrl.match(/\.md$/i) && !labUrl.match(/\.markdown$/i)) {
            if (!labUrl.endsWith('/')) {
                labUrl += '/';
            }
            labUrl += 'README.md';
    ...
    fetch(labUrl)
        ...
        .then(markdown => {
            ...
            blockquotes.forEach(bq => {
                ...
                if (firstP) {
                    ...
                    if (match) {
                        ...
                        // Remove the [!TYPE] string and any trailing break line
                        firstP.innerHTML = firstP.innerHTML.substring(match[0].length).replace(/^<br\s*\/?>\s*/i, '');
                        
                        // Add a title element with exactly GitHub's SVG Octicons
            ...
            // Fix relative image paths to point to the correct GitHub repo path
            ...
            images.forEach(img => {
                ...
                if (src && !src.startsWith('http') && !src.startsWith('data:')) {
                    // Clean up leading slashes or relative dot-slashes if present
                    let cleanSrc = src;
            ...
            preBlocks.forEach(pre => {
                ...
                btn.addEventListener('click', () => {
                    ...
                    // Get text strictly from the code element so we don't copy the button text itself
            ...
            tocList.innerHTML = ''; // Clear prior contents
            
            headings.forEach((heading, index) => {
                // Ensure heading has an ID to link to
                if (!heading.id) {
                    // Create a URL-safe ID from the text content
                ...
                // Indent sub-headers slightly more depending on their hierarchy level
                a.style.paddingLeft = `${16 + ((level - 1) * 12)}px`;
                
                // Close the flyout menu whenever a link is clicked
        ...
        .catch(error => {
            ...
            contentDiv.innerHTML = `<div class="error">
                ...
                <em>Note: Make sure you are using the "Raw" URL for the GitHub file (e.g., https://raw.githubusercontent.com/... instead of https://github.com/...)</em>
            </div>`;
        });
});